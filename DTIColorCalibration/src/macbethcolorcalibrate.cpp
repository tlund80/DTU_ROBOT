...
void MacBethColorCalibrate::getColorCenters(cv::Mat& colorCenters, const int& ulx, const int& uly, const int& urx, const int& ury, const int& llx, const int& lly, const int& lrx, const int& lry) {
   ...
   double cornersArray[] = {llx, lly, 1.0, ulx, uly, 1.0, urx, ury, 1.0, lrx, lry, 1.0};
   ...
   cv::gemm( templateCorners, templateCornersT, 1, NULL, 0, temp1);

   cv::Mat temp2 = cv::Mat(3, 3, CV_64F);
   cv::invert(temp1, temp2);

   cv::Mat temp3 = cv::Mat(4, 3, CV_64F);
   cv::gemm( templateCornersT, temp2, 1, NULL, 0, temp3);

   cv::Mat transMatrix = cv::Mat(3, 3, CV_64F);
   cv::gemm( corners, temp3, 1, NULL, 0, transMatrix);

   //colorCenters = transMatrix*templateColorCenters;
  // cv::Mat colorCenters = cv::Mat(3, 24, CV_64F);
   cv::gemm(transMatrix, templateColorCentersT, 1, NULL, 0, colorCenters);
}
...
/*
      for (int j = int(round(colorCenters.at<double>(0,i) - intervalMargin)) - 1; j < int(round(colorCenters.at<double>(0,i) + intervalMargin)); j++) {
         for (int k = int(round(colorCenters.at<double>(1,i) - intervalMargin)) - 1; k < int(round(colorCenters.at<double>(1,i) + intervalMargin)); k++) {
           //  std::cout << "(j,k) " << j << " " << k << std::endl;
          //  std::cout << (double)Image.data[j,k] << std::endl;
          //   cv::Vec3b rgb = Image.at<cv::Vec3b>(j,k);
          //   std::cout << "(r,g,b) " << rgb.val[0] << " " << rgb.val[1] << " " << rgb.val[2] << std::endl;
            rigColors.at<double>(i,0) = rigColors.at<double>(i,0) + double( (uchar) Image.data[Image.channels()*(Image.cols * j + k) + 0] / (pow(2.*intervalMargin + 1., 2.)));
            rigColors.at<double>(i,1) = rigColors.at<double>(i,1) + double( (uchar) Image.data[Image.channels()*(Image.cols * j + k) + 1] / (pow(2.*intervalMargin + 1., 2.)));
            rigColors.at<double>(i,2) = rigColors.at<double>(i,2) + double( (uchar) Image.data[Image.channels()*(Image.cols * j + k) + 2] / (pow(2.*intervalMargin + 1., 2.)));
...
cv::Mat  MacBethColorCalibrate::computeNormalization(cv::Mat image, int whiteRegionX, int whiteRegionY, int blackRegionX, int blackRegionY, int l_margin, int h_margin) {
  ...
  //std::cout<<"whiteValue : "<<whiteValue<<" blackValue : "<<blackValue<<std::endl;
...
void MacBethColorCalibrate::calibrate(cv::Mat input){
  ...
  //  cv::Vec3b* pixel = input.ptr<cv::Vec3b>(i); // point to first pixel in row
    for (int j = 0; j < input.cols; ++j){
        ...
        RGB_vector.at<double>(0) = double(pixel.val[0]);
         ...
         cv::gemm(RGB_vector, calibration, 1, NULL, 0, RGB_calibrated);
...
cv::Mat MacBethColorCalibrate::computeColorCalibration(cv::Mat& inputImage, int colorspace, int ulx, int uly, int urx, int ury, int llx, int lly, int lrx, int lry) {
