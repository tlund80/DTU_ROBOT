...
MacBethColorCalibrate::MacBethColorCalibrate() : counterL1(0), counterL2(0), counterR1(0), counterR2(0), LEFT_IMAGE_ID(1), RIGHT_IMAGE_ID(2)
{
    bigger = 0;
    smaller = 0;
...
void MacBethColorCalibrate::getColorCenters(cv::Mat& colorCenters, const int& ulx, const int& uly, const int& urx, const int& ury, const int& llx, const int& lly, const int& lrx, const int& lry) {
//   double templateCornersArray[] = {5.0 - 1, 239.0 - 1, 1.0, 5.0 - 1, 5.0 - 1, 1.0, 325.0 - 1, 5.0 - 1, 1.0, 325.0 - 1, 239.0 - 1, 1.0};
   ...
   double cornersArray[] = {double(llx), double(lly), 1.0, double(ulx), double(uly), 1.0, double(urx), double(ury), 1.0, double(lrx), double(lry), 1.0};
...
cv::Mat  MacBethColorCalibrate::getColorsFromRig(const cv::Mat &Image,  cv::Mat colorCenters) {
   ...
   //   std::cout << i <<"-bgr color-> (" << mean_color.val[2] << "," << mean_color.val[1] << "," << mean_color.val[0] << ")" << std::endl;
...
cv::Mat  MacBethColorCalibrate::computeNormalization(cv::Mat image, cv::Point2i whiteRegion, cv::Point2i blackRegion, int l_margin, int h_margin) {
   ...
   for (int r = whiteRegion.y - 5; r <= whiteRegion.y + 5; r++) {
      for (int c = whiteRegion.x - 5; c <= whiteRegion.x + 5; c++) {
   ...
   double blackValue = 0;
   counter = 0;
   for (int r = blackRegion.y - 5; r <= blackRegion.y + 5; r++) {
      for (int c = blackRegion.x - 5; c <= blackRegion.x + 5; c++) {
  ...
  std::cout<<"Mean white value : "<<whiteValue << std::setw(5) << " || " << std::setw(5) <<"Mean black value : "<<blackValue<<std::endl;
...
void MacBethColorCalibrate::colorNormalize(cv::Mat image, cv::Mat &normalizedColorImage, cv::Point2i whiteRegion,  cv::Point2i blackRegion, int l_margin, int h_margin) {

    cv::Mat ScalANDTrans = computeNormalization(image,whiteRegion,blackRegion,l_margin,h_margin);
    image.convertTo(normalizedColorImage,image.type(),ScalANDTrans.at<double>(0), ScalANDTrans.at<double>(1));
 //   image.convertTo(normalizedColorImage,image.type());
}
...
void MacBethColorCalibrate::colorCalibrate(cv::Mat input, cv::Mat output, cv::Mat calibration) {
   ...
   if (input.channels()  != output.channels() || input.channels() != calibration.rows)
      std::cout << "MacBethColorCalibrate::colorCalibrate(): There seems to be a problem with dimensions on input data." << std::endl;
...
cv::Mat MacBethColorCalibrate::computeColorCalibration(cv::Mat& inputImage, int colorspace, int ulx, int uly, int urx, int ury, int llx, int lly, int lrx, int lry,int camLorR) {
...
*/
 ...
 //  cv::Mat inverstMatrix(3, 24, CV_64F);
   ...
   cv::Mat rigColors(24, 3, CV_64F);
   rigColors = getColorsFromRig(inputImage, colorCenters);      //The 24 rig colors are extracted from the picture
...
*/
   ...
   */
   ...
   //Store local copy
   calibration_matrix[camLorR] = calibrationMatrix;

   return calibrationMatrix;
}

void MacBethColorCalibrate::getError(cv::Mat inputImage, int colorspace, int ulx, int uly, int urx, int ury, int llx, int lly, int lrx, int lry, int ColumnError, int camLorR) {
   ...
   cv::Mat colorCenters = cv::Mat(3, 24, CV_64F);
   getColorCenters(colorCenters, ulx, uly, urx, ury, llx, lly, lrx, lry);
   cv::Mat rigColors = getColorsFromRig(inputImage, colorCenters);
   cv::Mat colorErrors(1, 24, CV_64F);

/*   cv::Mat trueRGBValuesT(24, 3, CV_64F);
   cv::Mat rigColorsT(3, 24, CV_64F);
   cv::transpose(rigColors, rigColorsT);
    cv::Mat trueRGBValuescv = cv::Mat(3, 24, CV_64F, trueRGBValues);
   cv::transpose(trueRGBValuescv, trueRGBValuesT);
...
*/
    ...
    std::cout << std::setw(10) << "Blue-O" << std::setw(10) << "Blue-C" << std::setw(10) << "Blue-Err" << std::setw(10) << "Red-O" << std::setw(10) << "Red-C" << std::setw(10) << "Red-Err" << std::setw(10) << "Green-O" << std::setw(10) << "Green-C"<< std::setw(10) << "Green-Err" << std::setw(20) << "Total-Err" << std::endl;
      ...
      for (int i = 0; i < 24; i++) {
         ...
         if (i < 9)
            ...
            std::cout << i +1 << std::setw(5);
              ...
              std::cout << trueRGBValues[0][i] << std::setw(10) << rigColors.at<double>(i,0) << std::setw(10) << double (trueRGBValues[0][i] - rigColors.at<double>(i,0)) << std::setw(10)
                        << trueRGBValues[1][i] << std::setw(10) << rigColors.at<double>(i,1) << std::setw(10) << double (trueRGBValues[1][i] - rigColors.at<double>(i,1)) << std::setw(10)
                        << trueRGBValues[2][i] << std::setw(10) << rigColors.at<double>(i,2) << std::setw(10) << double (trueRGBValues[2][i] - rigColors.at<double>(i,2)) << std::setw(20) <<  colorErrors.at<double>(i) << std::endl;
      }

   std::cout << std::setw(10) << "MEAN ERROR: " << mean/24.0 << std::endl;
...
void MacBethColorCalibrate::save(std::string path, int camLorR){
    ...
    cv::FileStorage fs;
     ...
     fs << "colormatrix" << calibration_matrix[camLorR];
     fs.release();
