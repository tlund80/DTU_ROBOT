...
output_dir=output/rgbd_scenes

# Positionals
queries=(`ls -v $data_dir/scenes/*.ply | awk 'NR % 2 == 1'`) # Odd indices
targets=(`ls -v $data_dir/scenes/*.ply | awk 'NR % 2 == 0'`) # Even
...
do
	...
	#gdb --args $exec_dir/$exec_name $query $target $pose_dir $pose_suffix $output_dir $options $flags $*
    IFS=$'\n'
done
