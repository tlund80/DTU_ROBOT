function tri = pointcloud2mesh(X, z_axis, stdtol)

% mesh = meshD(data, refNormal, stdTol)

% Author : Ajmal Saeed Mian {ajmal.mian@uwa.edu.au}
%           Computer Science. Univ of Western Australia
%
% Arguments : X - Nx3 vertex coordinates [x y z] of the pointcloud
%             z_axis - (optional) 1x3 vector in the sensor direction
%                         =[0 0 1] if the sensor looking towards the -z_axis
%             stdtol - (optional) tolerance for edge filtering. default is 0.6
%             
%
% Return : tri - triangles using index numbers of the vertices
%
%
...
edge_length = [sqrt(sum((X(tri(:,1),:) - X(tri(:,2),:)).^2,2)),...
        sqrt(sum((X(tri(:,2),:) - X(tri(:,3),:)).^2,2)),...
        sqrt(sum((X(tri(:,3),:) - X(tri(:,1),:)).^2,2))];

tri(:,4) = max(edge_length,[],2);

resolution = mean(edge_length(:));
stdeviation = std(edge_length(:));
filt_limit = resolution + stdtol * stdeviation;

big_triangles = find(tri(:,4) > filt_limit); %find index numbers of triagles with edgelength more than filtLimit
tri(big_triangles,:) = []; % remove all triangles with edgelength more than filtlimit
tri(:,4) = []; % remove the max edgeLength column

tri = tri(:,1:3); % AGB: Remove the edge lengths
