...
# Test parameters
dec=-1 # Decimation
rad=30,30,30,30,30 # Feature radius multiplier
...
output_dir=output/bologna2_fusion
...
# Options and flags
options="--pose-separator=$pose_separator --decimation=$dec --radius=$rad --resolution=$res --threshold=$thres --features=ecsad,hist128,rops,shot,si --metrics=L2_RATIO"
flags="--verbose --fusion"

# Start
$exec_dir/$exec_name "$objects" "$scenes" $pose_dir $pose_suffix $output_dir $options $flags $* #2> /dev/null

# gdb -ex r--args $exec_dir/$exec_name "$objects" "$scenes" $pose_dir $pose_suffix $output_dir $options $flags $*
